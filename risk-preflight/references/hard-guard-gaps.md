...
| production write, deploy, live theme publish | preflight script / deploy workflow | environment, diff, rollback, and applicable authorization |
| database migration or bulk update/delete | DB safety wrapper / dry-run script | affected row count, authorized bound, transaction or rollback path |
| money, orders, inventory, booking, notifications | domain guard / API wrapper | preview affected records and verify applicable authorization |

...
description: Use for /step-back-and-think, 退一步想, 大方向, 盲點, 卡住, or when local patches may hide the real bottleneck.
---
...
它也用來跳脫短期 context。當工程卡住、一直 patch、準備 report out、或要問外部 reviewer 時，先不要沿著眼前錯誤繼續挖。先問：現有 ctx 有沒有把我們帶歪？問題是不是被錯誤命名？真正需要外部或未來 agent 重新推理的是什麼？
...
- 我是不是被眼前 ctx、最近一次錯誤、最新 artifact、或目前 diff 綁架了？
- 這個問題在整個賽局裡真的重要嗎，還是只是最 visible？
- 這個問題的命名是不是錯的？真正問題是在 product、permission、state、interface、verification、ownership、還是 workflow？
...
- 如果要交給外部 0-context reviewer，哪些 context、證據、假說、問題才會讓他重新推理，而不是沿用我的局部偏見？
...
先 freeze 當前局部敘事，不急著修。

把目前 ctx 拆成三類：

- Current facts：已驗證事實，帶 evidence。
- Local story：我目前怎麼解釋這些事實。
- Context trap：這個解釋可能被哪些東西帶歪，例如最近一次錯誤、最新資料夾、單一 log、active diff、使用者上一句話、或舊 handoff。

然後重問問題：

- 如果我今天第一次看到這個專案，會怎麼描述真正卡點？
- 如果外部 0-context agent 要重新推理，我要問他什麼，而不是叫他幫我確認既有結論？
- 哪些「看起來該做」其實只是短期 ctx 推出的假任務？
- 現在應該是 coding、operator decision、external report、等待 evidence，還是停止擴張？

若目標是產出外部 0-context packet，先用這個技能整理大局與問題，再用 `$report-for-outsourcing` 產正式外送格式。
...
在進入設計前，列出可能被忽略的角度。至少掃一次：

- User / operator：使用者現在真正要決策什麼？
- Product：問題是否其實是 value、scope、UX、或流程定義錯？
- Permission / safety：現在是不是缺 approval，不是缺 code？
- State / ownership：是不是有多個 writer、多個 truth source、或舊 artifact 假裝最新？
- Interface：是不是介面太大，導致每次都要讀很多 context 才能安全行動？
- Verification：是不是完成很難證明，所以一直靠信心推進？
- Recovery：如果方向錯，能不能便宜回退？
- External reviewer：外部 0-context agent 最可能看見哪個本地 agent 看不到的盲點？

輸出時要把 `Blind spots to check` 和 `Better questions` 分開。不要把盲點直接當結論。
...
Context escape：
- Current local story:
- Possible context trap:
- Blind spots to check:
- Better questions:
- Should this become report-out / external review:
...
- 先檢查短期 ctx 是否把問題命名帶歪；不要直接沿最新錯誤繼續 patch。
- 盲點先列為待查問題，不要直接包裝成結論。
- Report-out / external review 前先整理大局、證據、假說、卡點與好問題；正式外送格式交給 `$report-for-outsourcing`。
- 高可見問題不自動等於高優先權。

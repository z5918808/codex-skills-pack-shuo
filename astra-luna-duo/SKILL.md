---
name: astra-luna-duo
description: "Astra-led DUO: small work stays with Main; substantial execution uses one Luna max task or up to five with user-approved DAG."
---
...
Apply [Task sizing and opt-in DAG Workers](../duo-run/references/dag-workers.md) before dispatch: small work stays with Main, substantial serial work uses one Worker, and worthwhile independent branches require user DAG approval before using up to five persistent Luna max slots. In parallel mode A aggregates original evidence from B–E and reports to Astra, within the same five-Worker cap; it gains no review/dispatch authority. This is the shared canonical allocation/relay/lifecycle contract; pin it with this skill. No Hange, Sol, extra reviewer, subagent, CLI agent or substitute controller. Explicit execution invocation requests serial execution; parallel user-visible tasks additionally require goal-scoped DAG approval; apply existing model dispatch gates with original invocation evidence, actual route/settings and required approval records. Skill installation alone authorizes no dispatch. Gate failure blocks dependent dispatch, not independent preparation.
...
Astra alone writes one reusable fixed root `<workspace>/DUO_ASTRA_GOAL.md` plus the used fixed per-slot assignment files defined in the shared DAG contract; respect explicitly selected existing paths. No native /goal or create_goal. The root records run identity, scope/authority, protected state, full acceptance, rules hashes and run stop path. Slot files record assignment generations, steps/limits, return address and separate assignment stop paths. Mutable role/task IDs and delivery receipts stay in the existing receipt outside pinned goal/rules. Complete the root and that slot assignment before Worker creation; initial message includes role/slot/node, mission, boundaries, both absolute goal paths/hashes, generation and read-before-action instructions. No setup-only Worker or new goal file per batch.

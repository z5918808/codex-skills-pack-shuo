...
Apply [Task sizing and opt-in DAG Workers](../duo-run/references/dag-workers.md) before dispatch: small work stays with Main, substantial serial work uses one Worker, and worthwhile independent branches require user DAG approval before using up to five persistent Luna max slots. This is the shared canonical allocation/lifecycle contract; pin it with this skill. No Hange, Sol, extra reviewer, subagent, CLI agent or substitute controller. Explicit execution invocation requests serial execution; parallel user-visible tasks additionally require goal-scoped DAG approval; apply existing model dispatch gates with original invocation evidence, actual route/settings and required approval records. Skill installation alone authorizes no dispatch. Gate failure blocks dependent dispatch, not independent preparation.

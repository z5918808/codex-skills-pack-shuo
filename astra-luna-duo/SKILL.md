...
Astra alone writes one reusable fixed `<workspace>/DUO_ASTRA_GOAL.md`; respect an explicitly selected existing path. No native /goal or create_goal. The contract records task/run/generation and assignment IDs, role IDs/settings, scope/authority, protected state, full acceptance, exact execution limits, rules hashes, return tool/address, a generation-bound absolute stop-record path separate from pinned goal/rules, and existing compact receipt location. Complete it before Worker creation; initial message includes role, mission, boundaries, absolute goal path/hash, generation and read-before-action instructions. No setup-only Worker or new goal file per batch.

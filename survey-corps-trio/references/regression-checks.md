...
- Allocation: executable work starts with Levi; Astra owns direction, diagnosis and prompt corrections. Initial failure is baseline. Only three consecutive comparable normal executions after substantive prompt corrections without meaningful acceptance-quality improvement permit Hange handoff. Improvement resets the count; unclear evidence or tool/data/permission failures do not establish model inadequacy. No invented test batch, unsafe retry, duplicate completed work or assignment of reasoning to Luna. Duo never summons Hange, including after three failures.
...
- Delivery: local final alone fails. Confirmed send ends the sender turn; one corrected retry only after definite rejection. Ambiguous delivery has no blind retry. Stale events cannot duplicate work. Every role return ends「依整體目標與目前進度，我下一步應完成哪個具體成果？」.
- Continuation: Erwin handles a valid return with actual work/dispatch, an exact blocker or accepted completion. No acknowledgement loop, invented hook or polling. Sent is not proof of wake-up.
- Stop: latched user revocation prevents successors; reconcile owned effects and require fresh resume authority. Hash mismatch blocks dependent work; no silent contract overwrite.
- Clones: only separately authorized clones; report through the original parent. Parent reconciles coverage and promptly escalates blockers without duplicate forwarding.

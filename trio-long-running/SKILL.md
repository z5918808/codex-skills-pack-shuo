...
Large coupled analysis belongs to Astra planning before Sol develops a full solution. Trigger on interdependent modules, global topology, cross-domain tradeoffs or architecture-wide restructuring, not item count alone. Sol Main gathers only enough facts to frame the problem and sends it to the existing Astra Thinker; Astra Main owns this planning directly. Astra establishes the framework, dependencies, priorities and decomposition; Sol deepens bounded parts and implements authorized engineering, while Luna executes explicit steps. Do not wait for Sol failure or have Astra redo a completed Sol-wide analysis. This is the user's allocation preference, not a universal capability claim or permission to raise effort, replace Main or add a role.

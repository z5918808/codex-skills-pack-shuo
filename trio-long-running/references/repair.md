...
For unclear direction, repeated repair loops, excessive preparation, or disputed scope, Main gathers bounded facts and sends the short fixed prompt in SKILL.md's strategic step-back section to the same Thinker before developing a full solution. Astra challenges the framing directly; TRIO does not require it to read or invoke step-back-and-think. Main's explanation is a hypothesis, not the advisor's premise. Do not delegate after already completing the same analysis. Immediate safety handling and routine fixes with decisive evidence stay with Main; Thinker recommendations return to Main for scope reconciliation and authorized implementation. Explicit repo-required Main-side skill use remains applicable; the concise facts/story/context-trap/priority/owner prompt does not add a duplicate deep analysis or artifact requirement.
...
3. If material uncertainty remains about the repaired route or safe execution, send that bounded question to the same Astra Thinker with the evidence, then yield. Routine decisive repairs need no extra mid-run consultation; closing follows the risk-based acceptance route. Astra ready does not grant user permission.
...
5. Worker executes and returns actual evidence through reporting.md. Reviewer applies acceptance.md: accept routine work directly or route the affected escalation to the same Astra role. Main-authored repairs still require independent behavioral evidence; decisive fixtures may suffice for a routine repair, while material residual risk or uncertainty requires Astra. Corrections follow the same ownership split.

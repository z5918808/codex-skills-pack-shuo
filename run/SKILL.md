---
name: run
description: Use for /run or $run when the user wants a bounded research pass followed by evidence-driven execution.
---
...
Research only enough to choose the right execution segment, then act to a rerunnable proof. Run is not native Codex Goal and must not shadow or recreate it.

## Entry Gate

Use only for explicit `/run`, `$run`, or a clear request for research-first execution. For ordinary continuation, keep the evidence pass proportional to what is uncertain; for long native Goal work use the app goal and `long-running-agent` route.

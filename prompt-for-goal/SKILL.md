...
description: "Use when the user invokes /prompt for goal, wants to shape a goal, build a forward-looking roadmap, stage work, define milestones, reduce token waste in /goal usage, decide what a goal should include, prepare staged execution, or asks for a terminal-ready /goal prompt. Default output is a copy-ready conversation brief with roadmap and active stage queue; include a terminal-ready /goal prompt only when explicitly requested."
---

# Prompt for Goal

Use this skill to shape a desired outcome into a forward-looking roadmap plus compact staged execution. It also supports terminal-ready `/goal` prompts when explicitly requested.

The job is not to execute the goal by default. The job is to decide the goal shape: what future state matters, what strategic road should get there, where to start, what evidence proves progress, and when the agent should stop.

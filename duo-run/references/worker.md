...
After the stop/terminal check, verify the fixed root goal and assigned slot `goal_path` against their dispatched run/assignment generations and SHA256 values at these same action boundaries. A missing or changed goal stops dependent work for handoff; never recreate it, edit it, or adopt a later generation without a fresh authorized dispatch. Reviewer alone writes, deletes, or reuses these files under [the DAG lifecycle](dag-workers.md); another slot assignment is never this Worker's authority.

...
Once repair is verified, use [lifecycle](lifecycle.md) for a prior goal when one exists, then [dispatch](dispatch.md) for exactly one fresh goal. Recheck the affected readiness evidence and acceptance discriminator; a patch-local green result does not establish that the dispatch route works. Refill other useful ready slots under the DAG contract, then yield; do not wait for the repair Worker. If the old action already completed, the new goal consumes its evidence and forbids duplicate execution.

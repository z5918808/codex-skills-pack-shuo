...
- **Reviewer:** the activation chat unless the user names another; retain the user-facing endpoint. Own direction, planning, diagnosis, shared/local repair, authority, lifecycle and acceptance. Recommend `gpt-6-sol/high` for a Sol Main or `gpt-6-astra/high` for an Astra Main; verify the actual selected model and supported effort, and never silently switch or create another Reviewer. Astra uses the same high default for team research/guidance/review and authorized local implementation/repair/self-review; preserve an explicitly selected supported effort. Preserve a supported Sol setting. Unknown/unavailable setting controls are not proof of compliance. Never inherit Worker production permissions.

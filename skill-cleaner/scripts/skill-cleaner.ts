...
function codexModelContext(modelName: string): {
  ...
  return { tokens: 272_000, source: "fallback:gpt-5.6-terra", effectivePercent: 95 };
}
...
function configState(): {
  ...
  if (!exists(config)) return { disabledPaths, disabledPlugins, enabledPlugins, hasPluginConfig: false };
  ...
  for (const line of lines) {
    ...
    if (block === "skill") {
      ...
      if (pathMatch) currentPath = expandHome(pathMatch[1] ?? pathMatch[2] ?? "");
    ...
    } else if (block.startsWith("plugin:") && /^enabled\s*=\s*true/.test(line)) {
      enabledPlugins.add(block.slice("plugin:".length));
    }
  }
  return { disabledPaths, disabledPlugins, enabledPlugins, hasPluginConfig };
}
...
function discoverSkills(): Skill[] {
  const { disabledPaths, disabledPlugins, enabledPlugins, hasPluginConfig } = configState();
  ...
  for (const root of discoverRoots()) {
    for (const file of walkFiles(root, (candidate) => path.basename(candidate) === "SKILL.md", 10)) {
      ...
      const notEnabledByPluginConfig =
        pluginPrefix != null &&
        hasPluginConfig &&
        ![...enabledPlugins].some((plugin) => disabledPluginMatches(plugin, pluginPrefix));
      ...
      const skill: Skill = {
        ...
        enabled: !disabledByPath && !disabledByPlugin && !notEnabledByPluginConfig,
